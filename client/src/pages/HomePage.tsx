			...
			<header className="sticky top-0 z-30 border-b border-border/70 bg-background/90 backdrop-blur">
				<div className="mx-auto flex h-14 w-full max-w-[1280px] items-center justify-between gap-2.5 px-4 sm:h-16 sm:gap-4 sm:px-6 md:px-8 lg:px-10">
					...
					<div className="flex items-center gap-1.5 sm:gap-2">
						<Link to="/auth">
							<Button variant="outline" size="sm" className="h-8 rounded-full border-border/80 bg-white/75 px-3 text-[11px] sm:h-9 sm:px-3.5 sm:text-sm">
								Log In
							</Button>
						</Link>
						<Link to="/map" onClick={resetMap}>
							<Button size="sm" className="h-8 rounded-full px-3 text-[11px] sm:h-9 sm:px-4 sm:text-sm">
								Start Free
							</Button>
						</Link>
					</div>
				...
				>
					...
					<div className="relative grid gap-5 sm:gap-6 md:grid-cols-[1.25fr_0.75fr] md:items-end md:gap-10">
						<div className="space-y-3.5 sm:space-y-5 md:space-y-6">
							...
							>
								Plan your next career move with a brighter map of real opportunities.
							</motion.h1>
							...
							>
								Vari blends role roadmaps, salary signals, and curated courses in one high-clarity workspace so you can compare paths and take action fast.
							</motion.p>
							...
							>
								...
								<div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
									<span className="text-[10px] font-semibold uppercase tracking-[0.11em] text-[#5f4a3a] sm:text-xs sm:tracking-[0.13em]">Popular:</span>
									{popularTags.map((tag) => (
										<span key={tag} className="rounded-full border border-[#f4d8bf] bg-[#fff7edcc] px-2 py-0.5 text-[10px] font-semibold text-[#6d4a2f] sm:px-3 sm:py-1 sm:text-xs">
											{tag}
										</span>
									))}
								</div>
						...
						>
							<p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#865b35]">Live catalog</p>
							<p className="mt-2 text-2xl font-bold sm:text-3xl">{roles.length}</p>
							<p className="text-xs text-[#5f5a54] sm:text-sm">active role blueprints</p>
							<div className="mt-4 h-px bg-[#f2ddc9]" />
							<div className="mt-4 flex items-start justify-between gap-3">
								<div>
									<p className="text-xl font-bold sm:text-2xl">{industries.length}</p>
									<p className="text-xs text-[#5f5a54] sm:text-sm">industry clusters</p>
								</div>
								<div className="rounded-xl border border-[#f3dcc6] bg-[#fff3e4] px-2.5 py-1.5 text-right sm:rounded-2xl sm:px-3 sm:py-2">
									...
									<p className="text-[10px] text-[#7d6f62] sm:text-xs">jobs + course signals</p>
								</div>
							</div>
						</motion.div>
					</div>
