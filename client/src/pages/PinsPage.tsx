...
export function PinsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', padding: '40px 24px', background: 'var(--bg-canvas)' }}>
      <h1 style={{ fontFamily: 'Outfit', fontSize: 28, marginBottom: 24 }}>My Saved Pins ⭐</h1>
      {pins.length === 0 ? (
        <p style={{ color: 'var(--text-secondary)', fontFamily: 'Inter' }}>
          You haven't pinned any roles or industries yet. Explore the
          <a href="/map" style={{ marginLeft: 4 }}>map</a> and click "Pin" on things you find interesting!
        </p>
      ) : (
